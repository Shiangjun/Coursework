...
void FileCheck(int argc, char *argv[]){
  ...
  if (argc != 3) {
    ...
    FILE *valid_check_nofly;

    valid_check_nofly = fopen(argv[1],"r");

    while (!feof(valid_check_nofly)){
   
	char *line = (char *) malloc(sizeof(char) * MAX_LINE_NUM);

        fgets(line, MAX_LINE_NUM, valid_check_nofly);

        if ((line[0] != '#')&&(line[0] != '\n')&&((int) strlen(line)!= 0)){

          int counter_space = 0;

	  for (int i = 0; i < strlen(line); i++) if (line[i] == ' ') counter_space++;         
    ...
    fclose(valid_check_nofly);    

    FILE *valid_check_flightplan;

    char *line = (char *) malloc(sizeof(char) * MAX_LINE_NUM);

    valid_check_flightplan = fopen(argv[2],"r");

    while (!feof(valid_check_flightplan)){
   

        fgets(line, MAX_LINE_NUM, valid_check_flightplan);
	    ...
	    if (!(((line[i]<=57)&&(line[i]>=48))||(line[i]==' ')||(line[i]=='\n'))) { 
          ...
          char *line_trim = (char *) malloc(sizeof(char) * (int) strlen(line));

          trim(line, line_trim);

          if ((valid_number_counter(line_trim) != 2)&&(valid_number_counter(line_trim) != 0)&&(strlen(line_trim)!=0)) {

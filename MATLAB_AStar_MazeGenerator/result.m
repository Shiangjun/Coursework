...
if ((xval == xTarget) && (yval == yTarget))
    ...
    p = plot(Optimal_path(j, 1) , Optimal_path(j, 2) , 'ko'); % plot target
    j = j - 1;
    ...
    plot(Optimal_path(:, 1) , Optimal_path(:, 2),'ko'); % plot the optimal path
    plot(Optimal_path(:, 1) , Optimal_path(:, 2),'k');
else
    pause(1);
